package ru.deelter.verify.discord.listeners;
...
public class DiscordVerifyListener extends ListenerAdapter {
    ...
    public void onMessageReceived(MessageReceivedEvent event) {
        ...
        if (player == null || !player.isOnline()) {
            MessageEmbed notify = new EmbedBuilder().setDescription(Config.MSG_DS_PLAYER_OFFLINE).setColor(Color.red).build();
        ...
        if (dPlayer.isLinked()) {
            MessageEmbed notify = new EmbedBuilder().setDescription(Config.MSG_DS_ACCOUNT_EXISTS).setColor(Color.red).build();
            channel.sendMessageEmbeds(notify).queue(msg -> msg.delete().queueAfter(4, TimeUnit.SECONDS));
            return;
        }

        if (ApplicationManager.has(player.getUniqueId())) {
            MessageEmbed notify = new EmbedBuilder().setDescription(Config.MSG_DS_APPLICATION_EXISTS).setColor(Color.red).build();
            channel.sendMessageEmbeds(notify).queue(msg -> msg.delete().queueAfter(4, TimeUnit.SECONDS));
            return;
        }

        ApplicationManager.add(player.getUniqueId(), user.getIdLong());
        ...
        MessageEmbed notify = new EmbedBuilder().setDescription(Config.MSG_DS_APPLICATION_CREATED).setColor(Color.green).build();
        channel.sendMessageEmbeds(notify).queue(msg -> msg.delete().queueAfter(5, TimeUnit.SECONDS));
